        ...
        with st.expander(mp.get("name"), expanded=True):
            ...
            for res_idx, result in enumerate(mp.get("results", [])):
                ...
                with st.form(key=f"res_form_{mp_uuid}_{res_idx}"):
                    ...
                    submitted = st.form_submit_button("💾 Save Table Data")
                    if submitted:
                ...
                if qlen > 0:
                    ...
                    sel = st.number_input("Select Row #", min_value=1, max_value=qlen, step=1, key=f"row_sel_{mp_uuid}_{res_idx}")
                    row_index = sel - 1
                    ...
                    for pid_idx, pid in enumerate(current_ids):
                        ...
                        pid["scheme"] = cols_id[0].text_input("Scheme", pid.get("scheme", ""), key=f"prop_scheme_{mp_uuid}_{res_idx}_{row_index}_{pid_idx}")
                        pid["value"] = cols_id[1].text_input("Value", pid.get("value", ""), key=f"prop_value_{mp_uuid}_{res_idx}_{row_index}_{pid_idx}")
                        pid["link"] = cols_id[2].text_input("Link", pid.get("link", ""), key=f"prop_link_{mp_uuid}_{res_idx}_{row_index}_{pid_idx}")
